...
class Preprocessor:
    ...
    def drop_columns(self, data, columns):
        ...
        try:
            ...
            self.logger.exception('Exception raised while Dropping Columns:'+str(e))
            raise Exception()
    ...
    def feature_encoding(self, data):
        try:
            ...
            for col in self.new_data.columns:
                self.new_data = pd.get_dummies(self.new_data, columns=[col], prefix=[col], drop_first=True) #Avoid dummy variable trap
    ...
    def preprocess_trainset(self):
        try:
            ...
            data = self.get_data()
            data = self.drop_columns(data, ['transaction_id', 'user_id', 'fecha', 'dispositivo', 'dcto', 'cashback'])
            cat_df = self.feature_encoding(data)
            data = pd.concat([data, cat_df], axis=1)
            data = self.drop_columns(data, ['género', 'establecimiento', 'ciudad', 'status_txn'])
            is_null_present = self.is_null_present(data)
            if (is_null_present):
                data = self.impute_missing_values(data)
            self.X, self.y = self.split_features_label(data, label_name='fraude')
